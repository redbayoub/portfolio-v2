...
const makeActive = (menu_links, link) =>
  menu_links[link] != null ? menu_links[link].classList.add("active") : "";
const removeActive = (menu_links, link) =>
  menu_links[link].classList.remove("active");
const removeAllActive = (sections, menu_links) =>
  [...Array(sections.length).keys()].forEach((link) =>
    removeActive(menu_links, link)
  );


export default function(window, menu_links, sections) {
  //----------- This code is executed on scroll --------------------
  ...
  if (current !== currentActive) {
    removeAllActive(sections, menu_links);
    currentActive = current;
    makeActive(menu_links, current);
  }
}
